...
def zscore_normalize(train_csv: Path, test_csv: Path, output_dir: Path):
    ...
    # Select numeric columns (exclude file_path and utt_id)
    feature_cols = train_df.select_dtypes(include='number').columns.tolist()
    feature_cols = [c for c in feature_cols if c != 'tempo_std']  # or any non-feature numeric columns if needed
...
if __name__ == "__main__":
    ...
    zscore_normalize(processed_dir / "features_train.csv",
